...
class OfflineSyncInterceptor extends Interceptor {
  ...
  OfflineSyncInterceptor({required this.dio, required Box queueBox}) : _queueBox = queueBox {
    if (kDebugMode) print('[SYNC] Created.');
  }

  @override
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    final bool isOfflineSync = err.requestOptions.extra['isOfflineSync'] ?? true;
    final bool isSyncRequest = err.requestOptions.extra['isSyncRequest'] ?? false;

    // Prevent recursive queueing: don't queue if it's already a sync request
    if (!isSyncRequest &&
        _isNetworkError(err) &&
        ['POST', 'PUT', 'DELETE', 'PATCH'].contains(err.requestOptions.method) &&
        isOfflineSync) {
      if (kDebugMode) {
        print('[SYNC] Network error during mutation. Queuing request: ${err.requestOptions.uri}');
      }
  ...
  Future<void> _queueRequest(RequestOptions options) async {
    ...
    if (!exists) {
      if (kDebugMode) print('[SYNC] Queuing offline processed request: ${options.path}');
      await _queueBox.add(task);
    } else {
      if (kDebugMode) print('[SYNC] Duplicate request ignored: ${options.path}');
    }
  }
  ...
  Future<void> syncQueue() async {
    ...
    if (kDebugMode) print('[SYNC] Starting sync of ${_queueBox.length} items...');
    ...
    for (final key in keys) {
      ...
      try {
        if (kDebugMode) print('[SYNC] Processing ${task['method']} ${task['path']}');
